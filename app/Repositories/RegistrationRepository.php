...
final class RegistrationRepository
{
    ...
    private function scopeSql(int $userId, string $scope): array
    {
        ...
        if ($scope === 'organizer') return ['EXISTS (SELECT 1 FROM championship_user_assignments cua WHERE cua.championship_id = ar.championship_id AND cua.user_id = ? AND cua.assignment_type = \'organizer\')', [$userId]];
