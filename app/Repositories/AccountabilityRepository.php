...
final class AccountabilityRepository
{
    ...
    public function athleteFiles(int $championshipId): array
    {
        $statement = $this->pdo->prepare("SELECT a.id AS athlete_id, a.full_name AS athlete_name, t.name AS team_name, a.photo_path, ad.id AS document_id, ad.storage_path AS document_path, ad.original_name AS document_name, dt.name AS document_type FROM athlete_registrations ar INNER JOIN athletes a ON a.id = ar.athlete_id INNER JOIN teams t ON t.id = ar.team_id LEFT JOIN athlete_documents ad ON ad.athlete_id = a.id AND ad.deleted_at IS NULL AND ad.status = 'approved' LEFT JOIN athlete_document_types dt ON dt.id = ad.document_type_id WHERE ar.championship_id = ? AND ar.status = 'approved' ORDER BY t.name, a.full_name, ad.id");
        $statement->execute([$championshipId]);
        return $statement->fetchAll();
    }
