...
final class AccountabilityRepository
{
    ...
    public function summary(int $id): array
    {
        ...
        $queries = [
            'partidas_homologadas' => "SELECT COUNT(*) FROM matches WHERE championship_id = ? AND status = 'homologated'",
            'sumulas' => "SELECT COUNT(*) FROM match_report_versions v INNER JOIN matches m ON m.id = v.match_id WHERE m.championship_id = ?",
    ...
    {
        $sql = match ($kind) {
            'partidas' => "SELECT m.id, p.name AS fase, g.name AS grupo, ht.name AS mandante, at.name AS visitante, m.match_date AS data, m.match_time AS horario, m.status FROM matches m INNER JOIN competition_phases p ON p.id = m.phase_id INNER JOIN competition_groups g ON g.id = m.group_id INNER JOIN teams ht ON ht.id = m.home_team_id INNER JOIN teams at ON at.id = m.away_team_id WHERE m.championship_id = ? ORDER BY m.match_date, m.id",
            'atletas' => "SELECT a.full_name AS atleta, a.sporting_name AS nome_esportivo, t.name AS equipe, ar.status AS inscricao, ar.decided_at AS aprovado_em FROM athlete_registrations ar INNER JOIN athletes a ON a.id = ar.athlete_id INNER JOIN teams t ON t.id = ar.team_id WHERE ar.championship_id = ? ORDER BY t.name, a.full_name",
            'sumulas' => "SELECT m.id AS partida_id, ht.name AS mandante, at.name AS visitante, v.version_number AS versao, v.verification_code AS codigo, v.created_at AS gerada_em FROM match_report_versions v INNER JOIN matches m ON m.id = v.match_id INNER JOIN teams ht ON ht.id = m.home_team_id INNER JOIN teams at ON at.id = m.away_team_id WHERE m.championship_id = ? ORDER BY v.created_at DESC",
