...
final class AccountabilityRepository
{
    ...
    public function rows(int $id, string $kind): array
    {
        $sql = match ($kind) {
            ...
            'atletas' => "SELECT a.full_name AS atleta, a.sporting_name AS nome_esportivo, t.name AS equipe, ar.status AS inscricao, ar.approved_at AS aprovado_em FROM athlete_registrations ar INNER JOIN athletes a ON a.id = ar.athlete_id INNER JOIN teams t ON t.id = ar.team_id WHERE ar.championship_id = ? ORDER BY t.name, a.full_name",
