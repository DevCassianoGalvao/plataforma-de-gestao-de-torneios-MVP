...
final class PublicPortalRepository
{
    ...
    {
        $sql = "SELECT m.id, m.match_date, m.match_time, m.status, p.name AS phase_name, g.name AS group_name, r.round_number, v.name AS venue_name, ht.id AS home_team_id, ht.name AS home_team_name, ht.slug AS home_team_slug, ht.short_name AS home_team_short_name, ht.shield_path AS home_shield_path, at.id AS away_team_id, at.name AS away_team_name, at.slug AS away_team_slug, at.short_name AS away_team_short_name, at.shield_path AS away_shield_path, mo.administrative_home_score, mo.administrative_away_score, mo.first_half_started_at, mo.second_half_ended_at FROM matches m INNER JOIN competition_phases p ON p.id = m.phase_id INNER JOIN competition_groups g ON g.id = m.group_id INNER JOIN competition_rounds r ON r.id = m.round_id LEFT JOIN venues v ON v.id = m.venue_id INNER JOIN teams ht ON ht.id = m.home_team_id INNER JOIN teams at ON at.id = m.away_team_id LEFT JOIN match_operations mo ON mo.match_id = m.id WHERE m.championship_id = ? AND m.id = ? AND m.status NOT IN ('draft', 'cancelled') LIMIT 1";
