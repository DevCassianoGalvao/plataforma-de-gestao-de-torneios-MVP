<section>
    ...
    <?php if ($items === []): ?><p>Nenhum documento enviado.</p><?php else: ?><div class="table-wrap"><table><thead><tr><th>Tipo</th><th>Arquivo</th><th>Validade</th><th>Status</th><th>Motivo</th><th>Analise</th></tr></thead><tbody>
...
<section><h2>Enviar documento</h2><form method="post" enctype="multipart/form-data" action="<?= App\Core\View::e(App\Core\Config::url('/admin/atletas/' . $athlete['id'] . '/documentos')) ?>">
    ...
    <label>Responsavel, quando aplicavel <select name="guardian_id"><option value="">Nao se aplica</option><?php foreach ($guardians as $guardian): ?><option value="<?= (int) $guardian['id'] ?>"><?= App\Core\View::e($guardian['full_name']) ?> · <?= App\Core\View::e($guardian['relationship']) ?></option><?php endforeach; ?></select></label>
    <label>Arquivo <input type="file" name="document" accept=".pdf,.png,.jpg,.jpeg,.webp,application/pdf,image/png,image/jpeg,image/webp" required></label><label>Validade <input type="date" name="expires_at"></label><label>Observacao <textarea name="observation" rows="3"></textarea></label><button type="submit">Enviar para analise</button>
</form></section>
<?php endif; ?>
