<?php $initials = static function (string $name): string { $parts = preg_split('/\s+/', trim($name)) ?: []; $value = ''; foreach (array_slice($parts, 0, 2) as $part) { $value .= strtoupper(substr($part, 0, 1)); } return $value ?: 'EQ'; }; ?>
<section>
    ...
    <div class="table-wrap"><table><thead><tr><th>Equipe</th><th>Campeonato</th><th>Cidade</th><th>Treinador</th><th>Formação</th><th>Status</th><th>Ações</th></tr></thead><tbody>
    <?php foreach ($items as $item): ?><tr><td><span class="team-list-mark" aria-hidden="true"><?= App\Core\View::e($initials((string) $item['name'])) ?></span><strong><?= App\Core\View::e($item['name']) ?></strong><br><small><?= App\Core\View::e($item['abbreviation']) ?> · <?= App\Core\View::e($item['short_name']) ?></small></td><td><?= App\Core\View::e($item['championship_name']) ?></td><td><?= App\Core\View::e(trim(($item['city'] ?? '') . ' / ' . ($item['state'] ?? ''), ' /')) ?></td><td><?= App\Core\View::e($item['coach_name'] ?: 'Não definido') ?></td><td><?= App\Core\View::e($item['formation_name'] ?: 'Não definida') ?></td><td><span class="status status-<?= App\Core\View::e($item['status']) ?>"><?= App\Core\View::e($item['status']) ?></span></td><td><a href="<?= App\Core\View::e(App\Core\Config::url('/admin/equipes/' . $item['slug'])) ?>">Abrir</a></td></tr><?php endforeach; ?>
