<section>
    ...
    <div class="grid-links">
        ...
        <a class="link-card" href="<?= App\Core\View::e(App\Core\Config::url('/admin/campeonatos/' . $championship['slug'] . '/regulamento')) ?>"><strong>Regulamento</strong><span><?= $regulation ? App\Core\View::e($regulation['status'] . ' · versao ' . $regulation['version_number']) : 'Ainda nao criado' ?></span></a>
    ...
    <h2>Checklist</h2>
    <ul class="checklist"><li class="done">Informacoes gerais</li><li class="<?= $championship['logo_path'] || $championship['primary_color'] !== '#123C32' ? 'done' : '' ?>">Identidade basica</li><li class="<?= $regulation && $regulation['status'] === 'published' ? 'done' : '' ?>">Regulamento</li><li class="<?= $assignments !== [] ? 'done' : '' ?>">Organizadores vinculados</li><li class="pending">Equipes — ainda nao disponivel</li><li class="pending">Partidas — ainda nao disponivel</li></ul>
    <h2>Status</h2>
    <form method="post" action="<?= App\Core\View::e(App\Core\Config::url('/admin/campeonatos/' . $championship['slug'] . '/status')) ?>" class="inline-form"><input type="hidden" name="_csrf" value="<?= App\Core\View::e(App\Core\Security::csrfToken()) ?>"><label>Proxima etapa <select name="status"><option value="registration">Abrir inscricoes</option><option value="configured">Marcar configurado</option><option value="in_progress">Iniciar campeonato</option><option value="finished">Finalizar campeonato</option></select></label><button type="submit">Atualizar status</button></form>
