<section>
    ...
    <form class="search-form" method="get" action="<?= App\Core\View::e(App\Core\Config::url('/admin/campeonatos')) ?>">
        ...
        <label>Status <select name="status"><option value="">Todos</option><?php foreach (['draft' => 'Rascunho', 'registration' => 'Inscrições', 'configured' => 'Configurado', 'in_progress' => 'Em andamento', 'finished' => 'Finalizado', 'archived' => 'Arquivado'] as $key => $label): ?><option value="<?= $key ?>" <?= ($query['status'] ?? '') === $key ? 'selected' : '' ?>><?= $label ?></option><?php endforeach; ?></select></label>
    ...
    <div class="table-wrap"><table><thead><tr><th>Campeonato</th><th>Temporada</th><th>Categoria</th><th>Status</th><th>Visibilidade</th><th>Ações</th></tr></thead><tbody>
