                ...
                <div data-lineup-slots>
                    <?php foreach (($formation['slots'] ?? []) as $slot): ?>
                        ...
                        <div class="tactical-editor-slot" style="--slot-x: <?= (float) $slot['horizontal_position'] ?>%; --slot-y: <?= (float) $slot['vertical_position'] ?>%;">
                            ...
                            <label><span><?= App\Core\View::e($slot['label']) ?></span><select name="starters[<?= App\Core\View::e($slot['slot_key']) ?>]" aria-label="Selecionar atleta para <?= App\Core\View::e($slot['label']) ?>" <?= $lineup['status'] === 'draft' ? '' : 'disabled' ?>><option value="">Vazio</option><?php foreach ($athletes as $athlete): ?><option value="<?= (int) $athlete['id'] ?>" <?= $athleteId === (int) $athlete['id'] ? 'selected' : '' ?>><?= App\Core\View::e($displayName($athlete)) ?> #<?= App\Core\View::e($athlete['preferred_number'] ?: '-') ?></option><?php endforeach; ?></select></label>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
...
(() => {
    ...
    const render = (formationId) => {
        ...
        slotsRoot.innerHTML = formation.slots.map((slot) => {
            ...
            return `<div class="tactical-editor-slot" style="--slot-x:${slot.horizontal_position}%;--slot-y:${slot.vertical_position}%;"><div class="tactical-player${compatible ? '' : ' is-out-of-position'}">${avatar}<small>${escapeHtml(athlete?.name || 'Vazio')}</small></div><label><span>${escapeHtml(slot.label)}</span><select name="starters[${escapeHtml(slot.slot_key)}]" aria-label="Selecionar atleta para ${escapeHtml(slot.label)}">${options}</select></label></div>`;
        }).join('');
