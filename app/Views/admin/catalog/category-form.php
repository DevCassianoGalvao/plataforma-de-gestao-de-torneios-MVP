<section>
    ...
    <form method="post" action="<?= App\Core\View::e(App\Core\Config::url($editing ? '/admin/categorias/' . $record['id'] : '/admin/categorias')) ?>">
        ...
        <label>Regra de genero <select name="gender_rule"><option value="">Não definida</option><option value="male" <?= ($record['gender_rule'] ?? '') === 'male' ? 'selected' : '' ?>>Masculino</option><option value="female" <?= ($record['gender_rule'] ?? '') === 'female' ? 'selected' : '' ?>>Feminino</option><option value="mixed" <?= ($record['gender_rule'] ?? '') === 'mixed' ? 'selected' : '' ?>>Misto</option></select></label>
