...
<section class="registrations-page">
    ...
    <?php if ($items === []): ?><div class="empty-state"><strong>Nenhuma inscrição encontrada.</strong><p>Altere os filtros ou crie uma nova inscrição para iniciar o fluxo.</p></div><?php else: ?><div class="table-wrap"><table><thead><tr><th>Atleta</th><th>Equipe</th><th>Campeonato</th><th>Número</th><th>Status</th><th>Atualizada</th><th>Ação</th></tr></thead><tbody><?php foreach ($items as $item): ?><tr><td><strong><?= $e($item['sporting_name'] ?: $item['athlete_name']) ?></strong><br><small><?= $e($item['athlete_name']) ?></small></td><td><?= $e($item['team_name']) ?></td><td><?= $e($item['championship_name']) ?></td><td><?= $e($item['requested_number'] ?: '-') ?></td><td><span class="status status-<?= $e($item['status']) ?>"><?= $e($statusLabels[$item['status']] ?? $item['status']) ?></span></td><td><?= $e($item['updated_at']) ?></td><td><a href="<?= $e(App\Core\Config::url('/admin/inscricoes/' . $item['id'])) ?>">Abrir</a></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
</section>
