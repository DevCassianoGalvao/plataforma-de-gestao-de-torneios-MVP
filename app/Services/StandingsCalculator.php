...
final class StandingsCalculator
{
    public function calculate(array $teams, array $matches, array $regulation, int $qualified): array
    {
        ...
        usort($rows, function (array $first, array $second) use ($criteria, $matches, $rows, $regulation): int {
            if ($first['points'] !== $second['points']) return $second['points'] <=> $first['points'];
            foreach ($criteria as $criterion) {
                $compare = $this->compare($first, $second, $criterion, $matches, $rows, $regulation);
        ...
        foreach ($rows as $index => &$row) {
            ...
            if ($index > 0 && $row['points'] === $rows[$index - 1]['points']) $row['separated_by'] = $this->separatedBy($rows[$index - 1], $row, $criteria, $matches, $rows, $regulation);
        }
        unset($row);
        return $rows;
    }
    ...
    private function separatedBy(array $a, array $b, array $criteria, array $matches, array $cluster, array $regulation = []): string
    { foreach ($criteria as $criterion) if ($this->compare($a, $b, $criterion, $matches, $cluster, $regulation) !== 0) return $criterion; return 'draw_lots'; }
    private function headToHead(int $first, int $second, array $matches, array $cluster, array $regulation = []): int
    {
        ...
        foreach ($matches as $match) {
            ...
            $winPoints = (int) ($regulation['points_win'] ?? 3);
            $drawPoints = (int) ($regulation['points_draw'] ?? 1);
            if ($homeScore > $awayScore) $score[$home]['points'] += $winPoints; elseif ($awayScore > $homeScore) $score[$away]['points'] += $winPoints; else { $score[$home]['points'] += $drawPoints; $score[$away]['points'] += $drawPoints; }
        }
