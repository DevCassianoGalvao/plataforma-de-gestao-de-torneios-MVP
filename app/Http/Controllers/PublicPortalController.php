...
final class PublicPortalController
{
    ...
    {
        $urls = []; foreach ($this->portal->publicChampionships() as $championship) { $base = Config::absoluteUrl('/campeonatos/' . rawurlencode($championship['slug'])); foreach (['', '/proximos-jogos', '/resultados', '/classificacao', '/grupos', '/mata-mata', '/equipes', '/atletas', '/artilharia', '/assistencias', '/cartoes', '/suspensoes', '/noticias', '/vai-e-vem', '/regulamento', '/campeao'] as $path) $urls[] = '<url><loc>' . $this->xml($base . $path) . '</loc><lastmod>' . $this->xml(substr((string) $championship['updated_at'], 0, 10)) . '</lastmod></url>'; } return new Response('<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">' . implode('', $urls) . '</urlset>', 200, ['Content-Type' => 'application/xml; charset=UTF-8']);
    }
