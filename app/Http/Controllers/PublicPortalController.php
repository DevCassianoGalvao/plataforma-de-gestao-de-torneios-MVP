...
final class PublicPortalController
{
    ...
    public function nextMatches(Request $request, array $params = []): Response { return $this->listPage($request, $params, 'Proximos jogos', 'next', $this->portal->nextMatches((int) $this->id($params[0] ?? ''), 200)); }
    ...
    public function simulator(Request $request, array $params = []): Response { $championship = $this->championship($params[0] ?? ''); if ($championship instanceof Response) return $championship; $id = (int) $championship['id']; return $this->page($request, 'Simulador', 'public/portal/simulator', ['championship' => $championship, 'simulator' => $this->simulation->viewData($id), 'projection' => $this->simulation->project($id, [])]); }
    ...
    public function officials(Request $request, array $params = []): Response
    {
        ...
        $officials = $this->portal->officials((int) $championship['id']);
        if ($officials === []) return Response::redirect(Config::url('/campeonatos/' . rawurlencode((string) $championship['slug'])));
        return $this->page($request, 'Arbitragem', 'public/portal/officials', ['championship' => $championship, 'officials' => $officials]);
    }
