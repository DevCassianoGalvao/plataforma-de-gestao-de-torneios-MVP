...
final class MatchMediaController extends Controller
{
    ...
    { return $this->change($request,$params,'evidence.upload',function(array $user,array $match,int $id){$item=$this->media->find($id);if(!$item||(int)$item['match_id']!==(int)$match['id']||!$this->media->submit($id))return false;$this->media->log((int)$match['id'],$id,(int)$item['checklist_item_id'],'enviado_para_revisao','Rascunho enviado para revisão.',(int)$user['id']);return true;},'Evidência enviada para revisão.'); }
