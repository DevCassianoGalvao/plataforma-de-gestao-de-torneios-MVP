...
final class NewsController extends Controller
{
    ...
    {
        $guard = $this->guard($request, 'content.manage'); if ($guard instanceof Response) return $guard; $article = $this->articleForUser($guard, (int) ($params[0] ?? 0)); $name = basename((string) ($params[1] ?? ''));
        if (!$article || !preg_match('/^[a-f0-9]{32}\.(jpg|png|webp)$/', $name) || !str_contains((string) $article['content'], '[[imagem:news/content/' . $name . ']]')) return Response::html('Imagem não encontrada.', 404);
        $file = $this->storage->read('news/content/' . $name); if (!$file) return Response::html('Imagem não encontrada.', 404); return new Response($file['body'], 200, ['Content-Type' => $file['mime'], 'Cache-Control' => 'private, no-store']);
    }
