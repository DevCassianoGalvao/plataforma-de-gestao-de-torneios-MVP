...
final class ChampionshipIntegrationTest
{
    public static function run(): void
    {
        ...
        assert_same(1, (int) $pdo->query('SELECT COUNT(*) FROM championship_user_assignments')->fetchColumn(), 'Seed duplicou vinculo');
        ...
        assert_true($access->canView($admin, (int) $championship['id']), 'Escopo do administrador falhou');
        $regulations = new RegulationRepository($pdo);
        $service = new RegulationService($pdo, $regulations, new AuditService($pdo));
        $draftId = $service->ensureDraft((int) $championship['id'], (int) $admin['id'], Request::fake('GET', '/regulamento'));
        $service->applyPreset((int) $championship['id'], (int) $admin['id'], Request::fake('POST', '/regulamento/preset'));
        $service->applyPreset((int) $championship['id'], (int) $admin['id'], Request::fake('POST', '/regulamento/preset'));
        assert_same($draftId, (int) $regulations->draft((int) $championship['id'])['id'], 'Preset criou versoes duplicadas');
        $published = $service->publish((int) $championship['id'], (int) $admin['id'], Request::fake('POST', '/regulamento/publicar'));
        ...
        $nextDraftId = $service->ensureDraft((int) $championship['id'], (int) $admin['id'], Request::fake('GET', '/regulamento/editar'));
        ...
        assert_true($service->save((int) $championship['id'], (int) $admin['id'], array_merge(['name' => 'Regulamento customizado', 'effective_from' => '2026-02-01'], $custom), Request::fake('POST', '/regulamento'))['ok'], 'Regulamento customizado nao foi salvo');
