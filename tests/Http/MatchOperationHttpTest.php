...
final class MatchOperationHttpTest
{
    public static function run(): void
    {
        ...
        $central = $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $matchId . '/operacao'));
        ...
        assert_same(403, $router->dispatch(Request::fake('POST', '/copa-online/admin/partidas/' . $matchId . '/operacao/evento', ['event_type' => 'occurrence', 'period' => 'other', '_csrf' => 'invalid']))->status, 'CSRF da operacao foi aceito');
        self::logout($router);

        self::login($router, 'operador@torneios.local');
        assert_same(200, $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $matchId . '/operacao'))->status, 'Operador nao visualizou a central atribuida');
        self::logout($router);

        self::login($router, 'organizador@torneios.local');
        assert_same(200, $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $matchId . '/operacao'))->status, 'Organizador nao visualizou a central autorizada');
        self::logout($router);
        ...
        assert_same(403, $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $foreignMatchId . '/operacao'))->status, 'IDOR da central foi aceito');
        self::logout($router);

        self::login($router, 'comunicacao@torneios.local');
        assert_same(403, $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $matchId . '/operacao'))->status, 'Comunicacao recebeu central operacional');
        self::logout($router);
    }
    ...
    {
        assert_same(302, $router->dispatch(Request::fake('POST', '/copa-online/logout', ['_csrf' => Security::csrfToken()]))->status, 'Logout da central falhou');
    }
}
