...
final class ChampionshipHttpTest
{
    public static function run(): void
    {
        ...
        $list = $router->dispatch(Request::fake('GET', '/copa-online/admin/campeonatos'));
        ...
        $season = $router->dispatch(Request::fake('GET', '/copa-online/admin/temporadas'));
        assert_same(200, $season->status, 'Temporadas nao abriram');
        $newSeason = $router->dispatch(Request::fake('POST', '/copa-online/admin/temporadas', ['_csrf' => Security::csrfToken(), 'name' => 'Temporada HTTP', 'year' => '2027', 'starts_at' => '2027-01-01', 'ends_at' => '2027-12-31', 'status' => 'draft']));
        ...
        $newCategory = $router->dispatch(Request::fake('POST', '/copa-online/admin/categorias', ['_csrf' => Security::csrfToken(), 'name' => 'Categoria HTTP', 'slug' => 'categoria-http', 'description' => '', 'minimum_age' => '', 'maximum_age' => '', 'gender_rule' => '', 'status' => 'active']));
        ...
        $invalid = $router->dispatch(Request::fake('POST', '/copa-online/admin/campeonatos', ['_csrf' => $csrf, 'name' => 'Incompleto']));
        assert_same(422, $invalid->status, 'Formulario invalido de campeonato foi aceito');
        $adminCreated = $router->dispatch(Request::fake('POST', '/copa-online/admin/campeonatos', ['_csrf' => Security::csrfToken(), 'name' => 'Campeonato HTTP Admin', 'short_name' => 'HTTP Admin', 'slug' => 'campeonato-http-admin', 'description' => 'Teste de criacao.', 'season_id' => $httpSeasonId, 'category_id' => $httpCategoryId, 'starts_at' => '2027-03-01', 'ends_at' => '2027-04-01', 'registration_starts_at' => '2027-02-01', 'registration_ends_at' => '2027-02-20', 'visibility' => 'private']));
        ...
        $assigned = $router->dispatch(Request::fake('GET', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026'));
        assert_same(200, $assigned->status, 'Organizador atribuido nao abriu campeonato');
        assert_same(200, $router->dispatch(Request::fake('GET', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026/regulamento'))->status, 'Organizador nao abriu resumo do regulamento');
        assert_same(200, $router->dispatch(Request::fake('GET', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026/regulamento/editar'))->status, 'Organizador nao abriu editor do regulamento');
        $preset = $router->dispatch(Request::fake('POST', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026/regulamento/preset', ['_csrf' => Security::csrfToken()]));
        assert_same(302, $preset->status, 'Organizador nao aplicou preset');
        $save = $router->dispatch(Request::fake('POST', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026/regulamento', ['_csrf' => Security::csrfToken(), 'name' => 'Regulamento HTTP', 'effective_from' => '2026-03-01', 'group_count' => '2', 'teams_per_group' => '5', 'qualified_per_group' => '4', 'group_rounds' => 'single', 'knockout_starts_at' => 'quarterfinals', 'final_format' => 'single_match', 'points_win' => '3', 'points_draw' => '1', 'points_loss' => '0', 'wo_winner_goals' => '3', 'wo_loser_goals' => '0', 'yellow_cards_for_suspension' => '2', 'yellow_suspension_matches' => '1', 'red_card_automatic_suspension' => 'on', 'red_card_suspension_matches' => '1', 'reset_cards_stage' => '', 'regular_time_minutes' => '40', 'halftime_minutes' => '10', 'substitutions_allowed' => '5', 'substitution_windows' => '3', 'extra_time_minutes' => '10', 'tiebreakers' => ['wins' => ['enabled' => 'on', 'priority' => '1'], 'goal_difference' => ['enabled' => 'on', 'priority' => '2']]]));
        assert_same(302, $save->status, 'Organizador nao salvou rascunho');
        assert_same(200, $router->dispatch(Request::fake('GET', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026/regulamento/revisar'))->status, 'Revisao do regulamento nao abriu');
        $publish = $router->dispatch(Request::fake('POST', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026/regulamento/publicar', ['_csrf' => Security::csrfToken()]));
        assert_same(302, $publish->status, 'Organizador nao publicou regulamento');
        assert_same(200, $router->dispatch(Request::fake('GET', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026/regulamento/versoes'))->status, 'Historico de regulamentos nao abriu');
        ...
        $identity = $router->dispatch(Request::fake('POST', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026/identidade', ['_csrf' => Security::csrfToken(), 'default_theme' => 'light', 'primary_color' => '#123C32', 'secondary_color' => '#245C4A', 'accent_color' => '#D9A441'], ['logo_path' => ['error' => UPLOAD_ERR_OK, 'size' => filesize($tmp), 'tmp_name' => $tmp, 'name' => 'logo.png']]));
        ...
        assert_same(200, $router->dispatch(Request::fake('GET', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026/assets/logo_path'))->status, 'Asset privado nao foi servido');
        ...
        $invalidUpload = $router->dispatch(Request::fake('POST', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026/identidade', ['_csrf' => Security::csrfToken(), 'default_theme' => 'light', 'primary_color' => '#123C32', 'secondary_color' => '#245C4A', 'accent_color' => '#D9A441'], ['logo_path' => ['error' => UPLOAD_ERR_OK, 'size' => filesize($invalidFile), 'tmp_name' => $invalidFile, 'name' => 'logo.png']]));
        assert_same(422, $invalidUpload->status, 'Upload HTTP invalido foi aceito');
        @unlink($invalidFile);
        $organizerCreated = $router->dispatch(Request::fake('POST', '/copa-online/admin/campeonatos', ['_csrf' => Security::csrfToken(), 'name' => 'Campeonato HTTP Organizador', 'short_name' => 'HTTP Org', 'slug' => 'campeonato-http-organizador', 'description' => 'Teste de escopo.', 'season_id' => $seasonId, 'category_id' => $categoryId, 'starts_at' => '2026-09-01', 'ends_at' => '2026-10-01', 'registration_starts_at' => '2026-08-01', 'registration_ends_at' => '2026-08-20', 'visibility' => 'private']));
        ...
        assert_same(403, $router->dispatch(Request::fake('GET', '/copa-online/admin/campeonatos/campeonato-http-admin'))->status, 'Organizador acessou campeonato sem vinculo');
        $logout = $router->dispatch(Request::fake('POST', '/copa-online/logout', ['_csrf' => Security::csrfToken()]));
        assert_same(302, $logout->status, 'Logout do organizador falhou');
        self::login($router, 'organizador-sem-acesso@torneios.local');
        $denied = $router->dispatch(Request::fake('GET', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026'));
        ...
        $trainer = $router->dispatch(Request::fake('GET', '/copa-online/admin/campeonatos'));
        ...
        assert_same(403, $router->dispatch(Request::fake('GET', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026/regulamento'))->status, 'Operador acessou regulamento');
        $logout = $router->dispatch(Request::fake('POST', '/copa-online/logout', ['_csrf' => Security::csrfToken()]));
        assert_same(302, $logout->status, 'Logout do operador falhou');
        self::login($router, 'comunicacao@torneios.local');
        assert_same(403, $router->dispatch(Request::fake('GET', '/copa-online/admin/campeonatos/copa-brasil-de-talentos-2026/regulamento'))->status, 'Comunicacao acessou regulamento');
    }
