...
final class LineupHttpTest
{
    public static function run(): void
    {
        ...
        $summary = $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $matchId . '/escalacoes'));
        assert_same(200, $summary->status, 'Central HTTP de escalacoes nao abriu');
        $edit = $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $matchId . '/escalacao/' . $teamId));
        ...
        assert_same(200, $router->dispatch(Request::fake('POST', '/copa-online/admin/partidas/' . $matchId . '/escalacao/' . $teamId . '/automatico', ['_csrf' => Security::csrfToken(), 'formation_id' => $formationId]))->status, 'Distribuicao HTTP falhou');
        assert_same(403, $router->dispatch(Request::fake('POST', '/copa-online/admin/partidas/' . $matchId . '/escalacao/' . $teamId, ['_csrf' => 'invalid']))->status, 'CSRF de escalacao foi aceito');
        self::logout($router);

        self::login($router, 'organizador@torneios.local');
        assert_same(200, $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $matchId . '/escalacoes'))->status, 'Organizador nao visualizou escalacoes');
        assert_same(403, $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $matchId . '/escalacao/' . $teamId))->status, 'Organizador recebeu edicao de escalacao');
        self::logout($router);

        self::login($router, 'operador@torneios.local');
        assert_same(200, $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $matchId . '/escalacoes'))->status, 'Operador nao visualizou escalacoes confirmadas');
        assert_same(403, $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $matchId . '/escalacao/' . $teamId))->status, 'Operador recebeu edicao de escalacao');
        self::logout($router);

        self::login($router, 'treinador@torneios.local');
        assert_same(200, $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $matchId . '/escalacao/' . $teamId))->status, 'Treinador nao abriu propria escalacao');
        ...
        assert_same(403, $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $foreignMatch . '/escalacoes'))->status, 'IDOR de escalacao foi aceito');
        self::logout($router);

        self::login($router, 'comunicacao@torneios.local');
        assert_same(403, $router->dispatch(Request::fake('GET', '/copa-online/admin/partidas/' . $matchId . '/escalacoes'))->status, 'Comunicacao recebeu escalacoes');
        self::logout($router);
    }
    ...
    {
        assert_same(302, $router->dispatch(Request::fake('POST', '/copa-online/logout', ['_csrf' => Security::csrfToken()]))->status, 'Logout de escalacao falhou');
    }
}
