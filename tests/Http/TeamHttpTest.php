...
final class TeamHttpTest
{
    public static function run(): void
    {
        ...
        assert_same(200, $router->dispatch(Request::fake('GET', '/torneio-online/admin/equipes'))->status, 'Administrador nao abriu equipes');
        $invalid = $router->dispatch(Request::fake('POST', '/torneio-online/admin/equipes', ['_csrf' => Security::csrfToken(), 'name' => 'Incompleta']));
        assert_same(422, $invalid->status, 'Formulario invalido de equipe foi aceito');
        $created = $router->dispatch(Request::fake('POST', '/torneio-online/admin/equipes', ['_csrf' => Security::csrfToken(), 'championship_id' => $championshipId, 'name' => 'Equipe HTTP Admin', 'short_name' => 'HTTP Admin', 'slug' => 'equipe-http-admin', 'abbreviation' => 'EHA', 'description' => 'Equipe criada no contrato HTTP.', 'city' => 'Sao Paulo', 'state' => 'SP', 'primary_color' => '#123C32', 'secondary_color' => '#D9A441', 'default_tactical_formation_id' => $formationId]));
        ...
        $invalidUpload = $router->dispatch(Request::fake('POST', '/torneio-online/admin/equipes/equipe-http-admin/identidade', ['_csrf' => Security::csrfToken(), 'primary_color' => '#245C4A', 'secondary_color' => '#D9A441'], ['shield' => ['error' => UPLOAD_ERR_OK, 'size' => filesize($invalidFile), 'tmp_name' => $invalidFile, 'name' => 'escudo.png']]));
        ...
        $assigned = $router->dispatch(Request::fake('POST', '/torneio-online/admin/equipes/equipe-http-admin/responsaveis', ['_csrf' => Security::csrfToken(), 'user_id' => $managerId, 'assignment_type' => 'manager', 'starts_at' => '2026-01-01']));
        ...
        $ended = $router->dispatch(Request::fake('POST', '/torneio-online/admin/equipes/equipe-http-admin/responsaveis/' . $assignmentId . '/encerrar', ['_csrf' => Security::csrfToken(), 'ends_at' => '2026-02-01']));
        ...
        $staffCreate = $router->dispatch(Request::fake('POST', '/torneio-online/admin/equipes/equipe-http-admin/comissao', ['_csrf' => Security::csrfToken(), 'staff_role_id' => $roleId, 'full_name' => 'Preparador HTTP', 'display_name' => 'Prep HTTP', 'email' => 'prep@example.test', 'status' => 'active', 'starts_at' => '2026-01-01']));
        ...
        assert_same(200, $router->dispatch(Request::fake('GET', '/torneio-online/admin/equipes/equipe-http-admin/comissao/' . $staffId . '/editar'))->status, 'Edicao da comissao nao abriu');
        ...
        assert_same(302, $router->dispatch(Request::fake('POST', '/torneio-online/admin/equipes/equipe-http-admin/restaurar', ['_csrf' => Security::csrfToken()]))->status, 'Restauracao de equipe falhou');
