...
final class StandingsHttpTest
{
    public static function run(): void
    {
        ...
        $response = $router->dispatch(new Request('GET', '/copa-online/admin/classificacao', ['phase_id' => (string) $phaseId]));
        ...
        assert_same(403, $router->dispatch(Request::fake('POST', '/copa-online/admin/classificacao/recalcular', ['phase_id' => $phaseId, '_csrf' => 'invalid']))->status, 'CSRF da classificacao foi aceito');
        self::logout($router);
        self::login($router, 'comunicacao@torneios.local');
        assert_same(403, $router->dispatch(new Request('GET', '/copa-online/admin/classificacao', ['phase_id' => (string) $phaseId]))->status, 'Comunicacao recebeu classificacao');
        self::logout($router);
    }
    ...
    {
        assert_same(302, $router->dispatch(Request::fake('POST', '/copa-online/logout', ['_csrf' => Security::csrfToken()]))->status, 'Logout da classificacao falhou');
        Session::destroy();
    }
}
