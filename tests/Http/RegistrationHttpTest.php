...
final class RegistrationHttpTest
{
    public static function run(): void
    {
        ...
        assert_same(200, $router->dispatch(Request::fake('GET', '/torneio-online/admin/inscricoes'))->status, 'Treinador nao abriu central de inscricoes');
        assert_same(200, $router->dispatch(Request::fake('GET', '/torneio-online/admin/inscricoes/elenco'))->status, 'Treinador nao abriu elenco oficial');
        assert_same(200, $router->dispatch(Request::fake('GET', '/torneio-online/admin/inscricoes/nova'))->status, 'Formulario de inscricao nao abriu');
        $created = $router->dispatch(Request::fake('POST', '/torneio-online/admin/inscricoes', ['_csrf' => Security::csrfToken(), 'championship_id' => $championshipId, 'team_id' => $target['team_id'], 'athlete_id' => $target['id'], 'requested_number' => '44', 'observations' => 'Inscricao HTTP']));
        ...
        assert_same(200, $router->dispatch(Request::fake('GET', '/torneio-online/admin/inscricoes/' . $registrationId))->status, 'Detalhe da inscricao nao abriu');
        ...
        assert_same(403, $router->dispatch(Request::fake('POST', '/torneio-online/admin/inscricoes/' . $registrationId . '/rejeitar', ['_csrf' => 'invalid', 'rejection_reason' => 'x']))->status, 'CSRF de inscricao foi aceito');
        self::logout($router);
        ...
        self::login($router, 'treinador@torneios.local');
        assert_same(403, $router->dispatch(Request::fake('GET', '/torneio-online/admin/inscricoes/' . $foreignId))->status, 'Treinador acessou inscricao de outra equipe');
        self::logout($router);
    ...
    {
        assert_same(302, $router->dispatch(Request::fake('POST', '/torneio-online/logout', ['_csrf' => Security::csrfToken()]))->status, 'Logout de inscricao falhou');
    }
}
